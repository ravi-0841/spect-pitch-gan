...
def _get_spect(filename, dim=8):
    ...
    if spect.shape[0] > 128:
        ...
        return [mfc1[q:q+128], mfc2[q:q+128], mfc3[q:q+128], \
                mfc4[q:q+128], mfc5[q:q+128], mfc6[q:q+128], \
                mfc7[q:q+128], mfc8[q:q+128]]
    else:
        return None


def _get_spect_no_abs(filename, dim=8):
    sr, data = scwav.read(filename=filename)
    data = np.asarray(data, np.float64)
    _, spect, _ = pw.wav2world(data, sr, frame_period=5)
    if spect.shape[0] > 128:
...
#    
#    dim = 8
#    for sampling in range(8):
#        for i in src_list:
#            src_futures.append(executor.submit(partial(_get_spect, i, dim)))
...
#    for sampling in range(8):
#        for i in tar_list:
#            tar_futures.append(executor.submit(partial(_get_spect, i, dim)))
