...
def train(train_dir, model_dir, model_name, random_seed, \
    ...
    lc_lm = "lp_"+str(lambda_cycle_pitch) \
            ...
            + str(lambda_identity_mfc) + '_pre_trained_pmdw_normalized_all_spk'
    ...
    """
    mfc_A_mean = np.mean(np.vstack(mfc_A_train), axis=0, keepdims=True)
    mfc_A_stdv = np.std(np.vstack(mfc_A_train), axis=0, keepdims=True)
    mfc_B_mean = np.mean(np.vstack(mfc_B_train), axis=0, keepdims=True)
    mfc_B_stdv = np.std(np.vstack(mfc_B_train), axis=0, keepdims=True)

    mfc_A_train = (mfc_A_train - mfc_A_mean) / mfc_A_stdv
    mfc_B_train = (mfc_B_train - mfc_B_mean) / mfc_B_stdv
    mfc_A_valid = (mfc_A_valid - mfc_A_mean) / mfc_A_stdv
    mfc_B_valid = (mfc_B_valid - mfc_B_mean) / mfc_B_stdv

    np.savez(os.path.join('./data', 'vesus_neu_ang_mfc_scalers.npz'), 
            mean_A=mfc_A_mean, stdv_A=mfc_A_stdv, 
            mean_B=mfc_B_mean, stdv_B=mfc_B_stdv)
    ...
    for epoch in range(1,num_epochs+1):
        ...
        if epoch%100 == 0:
            ...
            for i in range(mfc_A_valid.shape[0]):
                ...
                pylab.legend(loc=2)
                pylab.subplot(222)
                pylab.plot(mfc_A_valid[i,:,:].reshape(-1,), label='Mfc A')
                pylab.plot(gen_mfc_B[0,:,:].reshape(-1,), label='Mfc A2B')
                pylab.plot(mfc_B_valid[i,:,:].reshape(-1,), label='Mfc B')
                pylab.legend(loc=2)
                ...
                pylab.subplot(224)
                pylab.plot(mfc_B_valid[i,:,:].reshape(-1,), label='Mfc B')
                pylab.plot(gen_mfc_A[0,:,:].reshape(-1,), label='Mfc B2A')
                pylab.plot(mfc_A_valid[i,:,:].reshape(-1,), label='Mfc A')
                pylab.legend(loc=2)
...
#    validation_dir_default = './data/evaluation/'+emo_pair+"/"+emo_dict[emo_pair][0]+'_5'
    ...
    train(train_dir=train_dir, model_dir=model_dir, model_name=model_name, 
          ...
          pre_train='./model/cmu-arctic/lp_0.0001_lm_0.0001_lmo_0.01_supervised_pmd_normalized/cmu-arctic950.ckpt', 
