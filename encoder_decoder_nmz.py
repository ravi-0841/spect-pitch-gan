...
def mfc_to_spect(mfc_feats):
    time_axis = np.argmax(mfc_feats.shape)
    if time_axis == 1:
        mfc_feats = mfc_feats.T
    mfc_feats = np.copy(mfc_feats, order='C')
    spect = pw.decode_spectral_envelope(np.asarray(mfc_feats, np.float64), 
                                        16000, 1024)
    return spect
...
class AE(object):
    ...
    def model_initializer(self):
        ...
        self.embedding, self.prediction \
                = self.encoder(input_mfc=(self.input_mfc+self.additive_noise), 
...
if __name__ == '__main__':
    ...
    """
    mfc_A = (mfc_A - np.mean(mfc_A, axis=0, keepdims=True)) \
                / np.std(mfc_A, axis=0, keepdims=True)
    mfc_B = (mfc_B - np.mean(mfc_B, axis=0, keepdims=True)) \
                / np.std(mfc_B, axis=0, keepdims=True)
