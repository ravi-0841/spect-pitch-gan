...
def pitch_discriminator(input_pitch, reuse=False, 
        scope_name='pitch_discriminator'):

    # input_pitch has shape [batch_size, num_features, time]
    input_pitch = tf.transpose(input_pitch, perm=[0,2,1], 
            name='pitch_discriminator_input_transpose')
...
def mfc_discriminator(input_mfc, reuse=False, 
        scope_name='mfc_discriminator'):

    # input_pitch has shape [batch_size, num_features, time]
    input_mfc = tf.transpose(input_mfc, perm=[0,2,1], 
            name='mfc_discriminator_mfc_transpose')
    ...
    with tf.variable_scope(scope_name) as scope:
        ...
        d1 = downsample1d_block(inputs=h1_mfc_glu, filters=128, 
                kernel_size=3, strides=2, 
                name_prefix='downsample2d_block1_')
        d2 = downsample1d_block(inputs=d1, filters=256, 
                kernel_size=3, strides=2, 
