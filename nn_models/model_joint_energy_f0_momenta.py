...
from modules.modules_joint_pitch_energy import sampler_pitch, \
        sampler_energy, discriminator
...
#        tf.reset_default_graph()
    ...
    def build_model(self):
        ...
        self.discrimination_B_fake = self.discriminator(input_energy=tf.concat([self.energy_A_real, self.energy_A2B_fake], axis=1), 
                                                            input_pitch=tf.concat([self.pitch_A_real, self.pitch_A2B_fake], axis=1), 
                                                            reuse=False, scope_name='discriminator_A')

        self.discrimination_A_fake = self.discriminator(input_energy=tf.concat([self.energy_B_real, self.energy_B2A_fake], axis=1), 
                                                            input_pitch=tf.concat([self.pitch_B_real, self.pitch_B2A_fake], axis=1), 
        ...
        self.generator_loss = self.gen_disc_loss + self.lambda_cycle_pitch*self.cycle_loss_pitch \
        ...
        self.discrimination_input_A_real_B_fake \
            = self.discriminator(input_energy=tf.concat([self.energy_A_real, self.energy_B_fake], axis=1), 
        ...
        self.discrimination_input_A_fake_B_real \
            = self.discriminator(input_energy=tf.concat([self.energy_A_fake, self.energy_B_real], axis=1), 
                                input_pitch=tf.concat([self.pitch_A_fake, self.pitch_B_real], axis=1), 
                                reuse=True, scope_name='discriminator_A')

        self.discrimination_input_B_real_A_fake \
            = self.discriminator(input_energy=tf.concat([self.energy_B_real, self.energy_A_fake], axis=1), 
        ...
        self.discrimination_input_B_fake_A_real \
            = self.discriminator(input_energy=tf.concat([self.energy_B_fake, self.energy_A_real], axis=1), 
        ...
        self.discriminator_loss_A \
            = (utils.l1_loss(y=tf.zeros_like(self.discrimination_input_A_real_B_fake), 
                ...
                + utils.l1_loss(y=tf.ones_like(self.discrimination_input_A_fake_B_real), 
                            y_hat=self.discrimination_input_A_fake_B_real)) / 2.0
        self.discriminator_loss_B \
            = (utils.l1_loss(y=tf.zeros_like(self.discrimination_input_B_real_A_fake), 
                ...
                + utils.l1_loss(y=tf.ones_like(self.discrimination_input_B_fake_A_real), 
                                y_hat=self.discrimination_input_B_fake_A_real)) / 2.0
