...
class VariationalCycleGAN(object):
    ...
    def __init__(self, dim_pitch=1, dim_mfc=23, n_frames=128, 
        ...
        self.interp_mat_mel2hz = utils._interp_mat_mel2hz()
    ...
    def build_model(self):
        ...
        self.cycle_loss_mfc = (utils.spectral_loss(y=self.mfc_A_real, 
            y_hat=self.mfc_cycle_A2A, interp_mat=self.interp_mat_mel2hz) \
                    + utils.spectral_loss(y=self.mfc_B_real, y_hat=self.mfc_cycle_B2B, 
                        interp_mat=self.interp_mat_mel2hz)) / 2.0
        
        # Identity Loss
        self.identity_loss_mfc = (utils.spectral_loss(y=self.mfc_identity_A2B, 
            y_hat=self.mfc_B_real, interp_mat=self.interp_mat_mel2hz) \
                    + utils.spectral_loss(y=self.mfc_identity_B2A, y_hat=self.mfc_A_real, 
                        interp_mat=self.interp_mat_mel2hz)) / 2.0
