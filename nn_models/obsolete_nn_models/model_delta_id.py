...
class VariationalCycleGAN(object):
    ...
    def __init__(self, dim_pitch=1, dim_mfc=23, n_frames=128, 
        ...
        # Create the derivative operator for momenta
        self.first_order_diff_mat = np.eye(self.n_frames, dtype=np.float32)
        for i in range(1, self.n_frames):
            self.first_order_diff_mat[i-1,i] = -1

        # Create derivative operator for mfcc
        self.delta_matrix = utils.delta_matrix(time_steps=n_frames)
    ...
    def summary(self):
        ...
        with tf.name_scope('generator_summaries'):
            ...
            cycle_loss_mfc_summary = tf.summary.scalar('cycle_loss_mfc', 
                    self.cycle_loss_mfc)
            cycle_loss_mfc_derivative_summary = tf.summary.scalar('cycle_mfc_derivative', 
                    self.cycle_loss_mfc_derivative)
            identity_loss_mfc_derivative_summary = tf.summary.scalar('identity_mfc_derivative', 
                    self.identity_loss_mfc_derivative)
            ...
            generator_summaries = tf.summary.merge([cycle_loss_pitch_summary, 
                cycle_loss_mfc_summary, cycle_loss_mfc_derivative_summary, 
                identity_loss_mfc_derivative_summary, generator_loss_A2B_summary, 
