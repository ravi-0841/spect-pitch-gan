...
from modules.modules_supervised_pitch_mfc_discriminate import sampler, \
...
class VariationalCycleGAN(object):
    ...
    def build_model(self):
        ...
        self.pitch_discrimination_input_A_real_B_fake \
            = self.pitch_discriminator(input_pitch=tf.concat([self.pitch_A_real, 
                self.pitch_B_fake], axis=1), reuse=True, scope_name='pitch_discriminator_A')
        self.pitch_discrimination_input_A_fake_B_real \
            = self.pitch_discriminator(input_pitch=tf.concat([self.pitch_A_fake, 
                self.pitch_B_real], axis=1), reuse=True, scope_name='pitch_discriminator_A')

        self.pitch_discrimination_input_B_real_A_fake \
            = self.pitch_discriminator(input_pitch=tf.concat([self.pitch_B_real, 
                self.pitch_A_fake], axis=1), reuse=True, scope_name='pitch_discriminator_B')
        self.pitch_discrimination_input_B_fake_A_real \
            = self.pitch_discriminator(input_pitch=tf.concat([self.pitch_B_fake, 
                self.pitch_A_real], axis=1), reuse=True, scope_name='pitch_discriminator_B')
        
        # Compute pitch discriminator loss for backprop
        self.pitch_discriminator_loss_input_A_real \
            = l1_loss(y=tf.zeros_like(self.pitch_discrimination_input_A_real_B_fake), 
                    y_hat=self.pitch_discrimination_input_A_real_B_fake)
        self.pitch_discriminator_loss_input_A_fake \
            = l1_loss(y=tf.ones_like(self.pitch_discrimination_input_A_fake_B_real), 
                    y_hat=self.pitch_discrimination_input_A_fake_B_real)
        self.pitch_discriminator_loss_A = (self.pitch_discriminator_loss_input_A_real \
                                     + self.pitch_discriminator_loss_input_A_fake) / 2.0

        self.pitch_discriminator_loss_input_B_real \
            = l1_loss(y=tf.zeros_like(self.pitch_discrimination_input_B_real_A_fake), 
                    y_hat=self.pitch_discrimination_input_B_real_A_fake)
        self.pitch_discriminator_loss_input_B_fake \
            = l1_loss(y=tf.ones_like(self.pitch_discrimination_input_B_fake_A_real), 
                    y_hat=self.pitch_discrimination_input_B_fake_A_real)
        self.pitch_discriminator_loss_B = (self.pitch_discriminator_loss_input_B_real \
                                     + self.pitch_discriminator_loss_input_B_fake) / 2.0

        # Merge the two discriminators into one
        self.pitch_discriminator_loss = (self.pitch_discriminator_loss_A + self.pitch_discriminator_loss_B) / 2.0

        # Compute the mfc discriminator probability for pair of inputs
        ...
        self.mfc_discriminator_loss_input_A_real \
            = l1_loss(y=tf.zeros_like(self.mfc_discrimination_input_A_real_B_fake), 
                    y_hat=self.mfc_discrimination_input_A_real_B_fake)
        self.mfc_discriminator_loss_input_A_fake \
            = l1_loss(y=tf.ones_like(self.mfc_discrimination_input_A_fake_B_real), 
                    y_hat=self.mfc_discrimination_input_A_fake_B_real)
        self.mfc_discriminator_loss_A = (self.mfc_discriminator_loss_input_A_real \
                                     + self.mfc_discriminator_loss_input_A_fake) / 2.0

        self.mfc_discriminator_loss_input_B_real \
            = l1_loss(y=tf.zeros_like(self.mfc_discrimination_input_B_real_A_fake), 
                    y_hat=self.mfc_discrimination_input_B_real_A_fake)
        self.mfc_discriminator_loss_input_B_fake \
            = l1_loss(y=tf.ones_like(self.mfc_discrimination_input_B_fake_A_real), 
                    y_hat=self.mfc_discrimination_input_B_fake_A_real)
        self.mfc_discriminator_loss_B = (self.mfc_discriminator_loss_input_B_real \
                                     + self.mfc_discriminator_loss_input_B_fake) / 2.0

        # Merge the two discriminators into one
        self.mfc_discriminator_loss = (self.mfc_discriminator_loss_A + self.mfc_discriminator_loss_B) / 2.0
