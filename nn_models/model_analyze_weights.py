...
class VariationalCycleGAN(object):
    ...
    def __init__(self, dim_pitch=1, dim_mfc=23, n_frames=128, 
            ...
            sampler=sampler, mode='train', log_dir_name='no_name', 
            pre_train=None):
    ...
    def summary(self):
        ...
        with tf.name_scope('generator_summaries'):
            ...
            generator_loss_summary = tf.summary.scalar('generator_loss', 
                    self.gen_disc_loss)
            generator_momenta_loss_summary = tf.summary.scalar('momenta_loss', 
                    self.momenta_loss)
            ...
            generator_f0_weight_A_summary = tf.summary.scalar('generator_A_f0_weight', 
                    tf.reduce_sum(self.generator_grads[0][1]**2))
            generator_f0_weight_B_summary = tf.summary.scalar('generator_B_f0_weight', 
                    tf.reduce_sum(self.generator_grads[136][1]**2))
            generator_mfc_weight_A_summary = tf.summary.scalar('generator_A_mfc_weight', 
                    tf.reduce_sum(self.generator_grads[62][1]**2))
            generator_mfc_weight_B_summary = tf.summary.scalar('generator_B_mfc_weight', 
                    tf.reduce_sum(self.generator_grads[198][1]**2))
            ...
            generator_summaries = tf.summary.merge([cycle_loss_pitch_summary, 
                ...
                generator_momenta_loss_summary, generator_gradient_summary,
                generator_f0_weight_A_summary, generator_f0_weight_B_summary, 
                generator_mfc_weight_A_summary, generator_mfc_weight_B_summary])
        ...
        with tf.name_scope('discriminator_summaries'):
            ...
            discriminator_f0_weight_A_summary = tf.summary.scalar('discriminator_A_f0_weight', 
                    tf.reduce_sum(self.discriminator_grads[4][1]**2))
            discriminator_f0_weight_B_summary = tf.summary.scalar('discriminator_B_f0_weight', 
                    tf.reduce_sum(self.discriminator_grads[38][1]**2))
            discriminator_mfc_weight_A_summary = tf.summary.scalar('discriminator_A_mfc_weight', 
                    tf.reduce_sum(self.discriminator_grads[0][1]**2))
            discriminator_mfc_weight_B_summary = tf.summary.scalar('discriminator_B_mfc_weight', 
                    tf.reduce_sum(self.discriminator_grads[34][1]**2))
            ...
            discriminator_summaries = tf.summary.merge([discriminator_loss_A_summary, 
                ...
                discriminator_gradient_summary, discriminator_f0_weight_A_summary, 
                discriminator_f0_weight_B_summary, discriminator_mfc_weight_A_summary, 
                discriminator_mfc_weight_B_summary])
