...
def train(train_dir, model_dir, model_name, random_seed, \
            ...
            discriminator_learning_rate=1e-07, tf_random_seed=None, 
            emo_pair='neu-ang', fold=1, count=1, gender_shuffle=False):
    ...
    if gender_shuffle:
        if tf_random_seed:
            lc_lm = 'lp_'+str(lambda_cycle_pitch) \
                    ...
                    + '_gender_'+emo_pair+'_fold_'+str(fold)+'_run_'+str(count)+'_random_seed_'+str(tf_random_seed)
        else:
            lc_lm = 'lp_'+str(lambda_cycle_pitch) \
                    + '_le_'+str(lambda_cycle_energy) \
                    + '_li_'+str(lambda_identity_energy) \
        ...
        if tf_random_seed:
            lc_lm = 'lp_'+str(lambda_cycle_pitch) \
                    + '_le_'+str(lambda_cycle_energy) \
                    + '_li_'+str(lambda_identity_energy) \
                    + '_'+emo_pair+'_fold_'+str(fold)+'_run_'+str(count)+'_random_seed_'+str(tf_random_seed)
        else:
            lc_lm = 'lp_'+str(lambda_cycle_pitch) \
                    + '_le_'+str(lambda_cycle_energy) \
                    + '_li_'+str(lambda_identity_energy) \
    ...
    if tf_random_seed:
        folder_extension = 'sum_mfc_wstn_'+emo_pair+'_fold_'+str(fold)+'/'
    else:
    ...
    data_train = scio.loadmat(os.path.join(train_dir, emo_pair+'_fold_'+str(fold)+'.mat'))
    data_valid = scio.loadmat(os.path.join(train_dir, emo_pair+'_fold_'+str(fold)+'.mat'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gender_shuffle', type=bool, help="Gender shuffling for training data", 
            default=False)
    parser.add_argument('--fold', type=int, default=1, help="fold of the data, choose from 1-5")
    parser.add_argument('--count', type=int, default=1, help="# of reruns with same random seed")
    ...
    train(train_dir=train_dir, model_dir=model_dir, model_name=model_name, 
          ...
          fold=fold, count=count, gender_shuffle=gender_shuffle)
